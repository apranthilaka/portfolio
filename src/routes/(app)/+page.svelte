<div class="w-screen flex items-center justify-center">
    <div
        class="flex flex-col items-center justify-center h-screen w-full lg:w-[600px] px-10"
    >
        <div class="mb-4 flex flex-col items-center">
            <div class="font-medium text-lg text-gray-800">Hi! Im Aravinda</div>
            <div class=" text-gray-950 max-w-xl text-center">
                I design & develop things that look great and scale even better.
            </div>
        </div>
        <div class="text-center text-sm text-gray-600 mb-4">
            I focus on creating "Tailwind-ready" design systems in Figma that
            are built for flexibility. From there, I translate those designs
            into high-performance, functional components using SvelteKit. The
            proof is in the build: this entire portfolio is a live demonstration
            of my stack, developed with SvelteKit, hosted on Cloudflare, and
            powered by Supabase. <br /> <br /> Explore my design systems and developed
            components below.
        </div>
        <div class="mt-4 flex flex-row gap-2">
            <a
                class="border border-gray-300 text-sm px-5 h-8 flex items-center justify-center rounded-sm text-gray-600 hover:text-gray-900 hover:bg-gray-100"
                href="../cards/cardsa">Components</a
            >
            <a
                class="border border-gray-300 text-sm px-5 h-8 flex items-center justify-center rounded-sm text-gray-600 hover:text-gray-900 hover:bg-gray-100"
                target="_blank"
                href="https://github.com/apranthilaka">Forms</a
            >
            <a
                class="border border-gray-300 text-sm px-5 h-8 flex items-center justify-center rounded-sm text-gray-600 hover:text-gray-900 hover:bg-gray-100"
                target="_blank"
                href="https://www.figma.com/@aravinda">Other...</a
            >
        </div>
        <div
            class="mt-6 border border-red-200 rounded-md w-full px-4 py-3 text-xs bg-red-50 text-red-500 text-center flex items-center justify-center"
        >
            I'm currently polishing up my 2026 updates! Some components and
            designs are still behind the scenes, but the full collection will be
            live soon.
        </div>
    </div>
</div>
